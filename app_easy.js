...
let listaOrdenadaAlfabeticamente = movies.sort(function (a, b) {
	if (a.title > b.title) {
		return 1;
	}
	if (a.title < b.title) {
...
let listaCartelera = movies.map(function (e){
...
{
	totalDePeliculas: movies.length
},
{
...
http.createServer((req, res) => {
	...
	switch (req.url) {
		// Home
		case '/':
			res.end(JSON.stringify(seccionesHome, null, " "));
			break;
		// En cartelera
		case '/en-cartelera':
			res.end(JSON.stringify(seccionesCartelera, null, " "));
			break;
		// Ranking
		case '/mas-votadas':
